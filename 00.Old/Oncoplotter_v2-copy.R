...
if (file.exists('Clinical_annotation.txt')){
  ...
  categorical_features <- options_df$Value[36:40]
  
  continuous_features <- options_df$Value[41:45]
  continuous_features <- continuous_features[!continuous_features %in% c("NA", NA)]
  
  remove_na <- function(vec) {
    vec[vec != "NA"]
  }
  categorical_colors <- options_df$Value[46:95]
  ...
  continuous_colors <- options_df$Value[96:105]
